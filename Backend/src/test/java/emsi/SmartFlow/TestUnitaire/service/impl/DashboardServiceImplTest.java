...
class DashboardServiceImplTest {
    ...
    void getAdminReportForYear_shouldPopulateProjectsActivity() {
        ...
        List<Object[]> projRows = new ArrayList<>();
        projRows.add(new Object[]{2024, 3, 7L});
        when(projectRepository.countGroupedByYearMonth(any(), any())).thenReturn(projRows);
        when(taskRepository.countGroupedByYearMonth(any(), any())).thenReturn(List.of());
    ...
    void getAdminReportForYear_shouldPopulateTasksActivity() {
        ...
        // ✅ project rows must be stubbed BEFORE task rows
        List<Object[]> projRows = new ArrayList<>();
        when(projectRepository.countGroupedByYearMonth(any(), any())).thenReturn(projRows);

        // ✅ task rows stubbed AFTER project rows — this is what we assert on
        List<Object[]> taskRows = new ArrayList<>();
        taskRows.add(new Object[]{2024, 5, 12L});
        when(taskRepository.countGroupedByYearMonth(any(), any())).thenReturn(taskRows);

        // ✅ For year report, from/to span full year → no daily calls needed
        // Do NOT stub countGroupedByDay here — it won't be called

        AdminDashboardSummary result = dashboardService.getAdminReportForYear(2024);

        assertNotNull(result.getTasksActivity());
        assertEquals(1, result.getTasksActivity().size());        // ← this was failing
    ...
    void getAdminSummaryCurrentMonth_shouldPopulateDailyActivityForCurrentMonth() {
        ...
        List<Object[]> projDayRows = new ArrayList<>();
        projDayRows.add(new Object[]{15, 3L});
        when(projectRepository.countGroupedByDay(any(), any())).thenReturn(projDayRows);

        List<Object[]> taskDayRows = new ArrayList<>();
        taskDayRows.add(new Object[]{15, 8L});
        when(taskRepository.countGroupedByDay(any(), any())).thenReturn(taskDayRows);
